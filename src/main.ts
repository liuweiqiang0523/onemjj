import { tools, scripts, notes, type Tool } from './data';
import './style.css';
...
let selected: Tool = tools[0];

function filteredTools() {
  return active === '全部' ? tools : tools.filter(t => t.category === active);
}
...
function toolCard(t: Tool) {
  return `<button class="tool-card ${t.private ? 'is-private' : ''}" data-tool="${t.id}" data-category="${t.category}">
...
function renderHome() {
  return `<section class="hero glass"><div><span class="eyebrow">OneMJJ / Low-maintenance survival center</span><h1>一个 MJJ 的低维护自救中心</h1><p>VPS 检测、三网延迟、自托管入口、PT 媒体、AI API、常用脚本和传家宝笔记，放到一个双端都舒服的工具台。</p><div class="actions"><button data-mode="command">进入控制台</button><button class="ghost" data-mode="weekly">看 MJJ 小报</button></div></div><aside class="status-card"><b>今日状态</b><div><span class="dot ok"></span>onemjj.com 已上线</div><div><span class="dot ok"></span>Cloudflare Pages 正常</div><div><span class="dot warn"></span>真实监控待接入</div><code>https://onemjj.com</code></aside></section>
  <section class="toolbar"><button class="search" data-focus-search>⌘K 搜索：YABS、回程、Emby、Sub2API...</button><div class="chips">${categories.map(c=>`<button class="chip ${c===active?'active':''}" data-cat="${c}">${c}</button>`).join('')}</div></section>
  <section class="grid tools-grid">${filteredTools().map(toolCard).join('')}</section>
  <section id="scripts" class="scripts glass"><div><span class="eyebrow">Scripts</span><h2>脚本速查</h2><p>点击脚本卡片会复制命令；危险命令不上自动执行，只做速查。</p></div><div class="script-list">${scripts.map(s=>`<button class="script" data-copy="${escapeAttr(s.cmd)}"><b>${s.title}</b><code>${s.cmd}</code><span>点击复制</span></button>`).join('')}</div></section>`;
}

function renderCommand() {
  return `<section class="command-layout"><aside class="nav-panel glass"><b>OneMJJ</b>${['Command','VPS','Network','Selfhosted','Scripts','Notes'].map((n,i)=>`<a class="${i===0?'active':''}" data-cat="${n === 'Command' ? '全部' : n}">${n}</a>`).join('')}</aside><main class="console glass"><span class="badge">MJJ Command Center</span><h1>买鸡、测鸡、吃灰、复活。</h1><p>工具卡片现在都能点开详情；真实三网监控、私有入口后续再接入，避免公开泄露后台。</p><div class="grid mini">${filteredTools().slice(0,8).map(toolCard).join('')}</div><pre>$ onemjj check gd-cm\n广东移动 TCP 443: 待接入\n$ onemjj yabs oracle\n点击「VPS 检测」查看脚本</pre></main><aside class="metrics glass"><h3>实时面板</h3>${['onemjj.com Online','Pages OK','自托管入口 Hidden','真实监控 Planned','最近冲动消费 0 台'].map(x=>`<div><span>${x.replace(/ [^ ]+$/,'')}</span><b>${x.split(' ').at(-1)}</b></div>`).join('')}</aside></section>`;
}
...
function bind(){
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-tool]').forEach(el=>el.onclick=()=>{const found=tools.find(t=>t.id===el.dataset.tool); if(found){selected=found; mode='tool'; render(); window.scrollTo({top:0,behavior:'smooth'});}});
