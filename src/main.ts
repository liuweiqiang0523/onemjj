...
function renderHome() {
  return `<section class="hero glass"><div><span class="eyebrow">OneMJJ / Low-maintenance survival center</span><h1>一个 MJJ 的低维护自救中心</h1><p>VPS 检测、三网延迟、自托管入口、PT 媒体、AI API、常用脚本和传家宝笔记，放到一个双端都舒服的工具台。</p><div class="actions"><a class="primary-link" href="/admin/">进入控制台</a><button class="ghost" data-mode="weekly">看 MJJ 小报</button></div></div><aside class="status-card"><b>今日状态</b><div><span class="dot ok"></span>onemjj.com 已上线</div><div><span class="dot ok"></span>控制台可编辑</div><div><span class="dot warn"></span>真实监控待接入</div><code>公开工具：首页｜管理：控制台</code></aside></section><section class="toolbar"><button class="search" data-focus-search>⌘K 搜索：YABS、回程、Emby、Sub2API...</button><div class="chips">${cats().map(c=>`<button class="chip ${c===active?'active':''}" data-cat="${c}">${c}</button>`).join('')}</div></section><section class="grid tools-grid">${filteredTools().map(toolCard).join('')}</section><section id="scripts" class="scripts glass"><div><span class="eyebrow">Scripts</span><h2>脚本速查</h2><p>点击脚本卡片会复制命令；危险命令不上自动执行，只做速查。</p></div><div class="script-list">${siteData.scripts.map(s=>`<button class="script" data-copy="${escapeAttr(s.cmd)}"><b>${s.title}</b><code>${s.cmd}</code><span>点击复制</span></button>`).join('')}</div></section>`;
}
function renderCommand() {
  return `<section class="command-layout"><aside class="nav-panel glass"><b>OneMJJ</b>${['Command','VPS','Network','Selfhosted','Scripts','Notes'].map((n,i)=>`<a class="${i===0?'active':''}" data-cat="${n === 'Command' ? '全部' : n}">${n}</a>`).join('')}</aside><main class="console glass"><span class="badge">MJJ Command Center</span><h1>买鸡、测鸡、吃灰、复活。</h1><p>工具卡片都能点开详情；真实三网监控、私有入口后续再接入。</p><div class="grid mini">${filteredTools().slice(0,8).map(toolCard).join('')}</div><pre>$ onemjj check gd-cm\n广东移动 TCP 443: 待接入\n$ onemjj yabs oracle\n点击「VPS 检测」查看脚本</pre></main><aside class="metrics glass"><h3>实时面板</h3>${['onemjj.com Online','Pages OK','数据维护 Hidden','真实监控 Planned','最近冲动消费 0 台'].map(x=>`<div><span>${x.replace(/ [^ ]+$/,'')}</span><b>${x.split(' ').at(-1)}</b></div>`).join('')}</aside></section>`;
}
...
function render() {
  app.innerHTML = `<nav class="top"><a class="brand" data-mode="home">OneMJJ</a><div><button data-mode="home" class="${mode==='home'?'active':''}">首页</button><a class="nav-admin" href="/admin/">控制台</a><button data-mode="weekly" class="${mode==='weekly'?'active':''}">小报</button></div></nav><main>${mode==='home'?renderHome():mode==='command'?renderCommand():mode==='weekly'?renderWeekly():renderTool()}</main><footer>OneMJJ · 少踩坑，多留传家宝 · Public tools first, private links later.</footer>`;
  bind();
}
function bind(){
